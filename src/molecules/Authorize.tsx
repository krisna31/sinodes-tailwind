...
import ModalTooMuch from "../atoms/ModalTooMuch";
...
const Authorize = ({ getUserData }: { getUserData: UserDataType }) => {
  ...
  const [manyCard, setManyCard] = useState(0)

  const [showModalTooMuch, setShowModalTooMuch] = useState(false)
  ...
  useEffect(() => {
    ...
    onValue(notesRef, (snapshot) => {
      ...
      if (notes != null) {
        setManyCard(Object.keys(notes).length || 0)
  ...
  return (
    <>
      {isFetch ?
        ...
        : getNotes.isExist ?
          manyCard < maxCard || getUserData.email.match("krisnaaaputraaa@gmail.com") ?
            ...
            <div className="container min-w-full bg-slate-200 dark:bg-slate-400 flex-1 justify-center items-start flex flex-col relative">
              ...
              <span
                className="fixed rounded-full right-5 bottom-6 bg-slate-400 text-4xl p-5 cursor-pointer shadow-md hover:shadow-2xl hover:bg-red-500 flex justify-center items-center w-16 h-16 transition-all ease-in-out delay-75 duration-200 hover:scale-125 hover:-translate-y-4"
                onClick={() => setShowModalTooMuch(true)}
              >
                !!!
              </span>
              <ModalTooMuch showModalTooMuch={showModalTooMuch} setShowModalTooMuch={setShowModalTooMuch} maxCard={maxCard} />
            </div>
          : <div className="container min-w-full bg-slate-200 dark:bg-slate-400 flex-1 justify-center items-start flex flex-col relative">
